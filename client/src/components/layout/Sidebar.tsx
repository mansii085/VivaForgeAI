      ...
      <AnimatePresence>
        {!isSidebarCollapsed && (
          <motion.div
            ...
            animate={{ opacity: 1, height: 'auto' }}
            exit={{ opacity: 0, height: 0 }}
            className="mx-3 mt-3 mb-2 rounded-xl p-2.5 relative overflow-hidden"
            style={{
              background: 'rgba(255,255,255,0.02)',
              border: '1px solid var(--border-color)',
            }}
          >
            ...
            <div className="grid grid-cols-2 gap-1.5">
              <div className="flex flex-col bg-black/40 rounded-lg px-2 py-1.5 border border-white/5">
                <span className="text-[9px] font-semibold text-slate-400 leading-tight">Interviews</span>
                <span className="text-xs font-bold" style={{ color: 'var(--text-primary)' }}>
                  {dashboardData?.stats?.interviewCount !== undefined ? dashboardData.stats.interviewCount : '—'}
                </span>
              </div>
              <div className="flex flex-col bg-black/40 rounded-lg px-2 py-1.5 border border-white/5">
                <span className="text-[9px] font-semibold text-slate-400 leading-tight">Avg Score</span>
                <span className="text-xs font-bold" style={{ color: 'var(--text-primary)' }}>
                  {dashboardData?.stats?.avgInterviewScore !== undefined ? `${dashboardData.stats.avgInterviewScore}%` : '—'}
                </span>
              </div>
              <div className="flex flex-col bg-black/40 rounded-lg px-2 py-1.5 border border-white/5">
                <span className="text-[9px] font-semibold text-slate-400 leading-tight">Resume</span>
                <span className="text-xs font-bold" style={{ color: 'var(--text-primary)' }}>
                  {dashboardData?.stats?.resumeCompletion !== undefined ? `${dashboardData.stats.resumeCompletion}%` : '—'}
                </span>
              </div>
              <div className="flex flex-col bg-black/40 rounded-lg px-2 py-1.5 border border-white/5">
                <span className="text-[9px] font-semibold text-slate-400 leading-tight">Learning</span>
                <span className="text-xs font-bold" style={{ color: 'var(--text-primary)' }}>
                  {dashboardData?.stats?.learningProgress !== undefined ? `${dashboardData.stats.learningProgress}%` : '—'}
                </span>
              </div>
